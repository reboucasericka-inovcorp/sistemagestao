...
import { createRole, getPermissionsCatalog, getRoleById, updateRole } from '@/modules/access-management/roles/services/roleService'
import type { AccessRole, UpsertAccessRolePayload } from '@/modules/access-management/roles/types/role'
...
const { resetForm, setErrors } = useForm<RoleFormData>({
  validationSchema: toTypedSchema(roleSchema),
  initialValues: { name: '', is_active: true },
})

function applyBackendRole(payload: AccessRole): void {
...
watch(
  ...
  async (isOpen) => {
    ...
    feedbackMessage.value = ''
    if (isNew.value) {
      resetForm({ values: { name: '', is_active: true } })
      matrix.value = buildEmptyPermissionMatrix()
      return
    }
    await loadRoleIfEditing()
...
<template>
  <div class="page">
    ...
    <Form class="form" @submit="(values) => onSubmit(values as RoleFormData)">
        ...
        <table class="w-full">
          <thead>
            <tr>
              ...
              <th v-for="action in ACCESS_ACTIONS" :key="action" class="px-2 py-2 text-left">{{ action }}</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="moduleName in ACCESS_MODULES" :key="moduleName">
              <td class="px-2 py-2">{{ moduleName }}</td>
              <td v-for="action in ACCESS_ACTIONS" :key="`${moduleName}.${action}`" class="px-2 py-2">
                <Checkbox
