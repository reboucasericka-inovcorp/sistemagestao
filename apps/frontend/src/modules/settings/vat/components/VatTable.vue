...
import { listVatResult, toggleVatStatus, type VatListMeta } from '@/modules/settings/vat/services/vatService'
...
const pagination = reactive<VatListMeta>({
  current_page: 1,
  per_page: 15,
  total: 0,
  last_page: 1,
...
async function fetchVat(): Promise<void> {
  ...
  errorMessage.value = ''
  try {
    const result = await listVatResult({
      search: searchDebounced.value || undefined,
      page: pagination.current_page,
      per_page: filters.perPage,
    })
    vats.value = result.data
    ...
    vats.value = []
    errorMessage.value = 'Não foi possível carregar o IVA.'
  } finally {
    loading.value = false
  }
}
...
async function onToggleStatus(vat: Vat): Promise<void> {
  togglingId.value = vat.id
  try {
    await toggleVatStatus(vat.id, vat.is_active)
    await fetchVat()
  } finally {
  ...
  <div class="space-y-4">
    ...
    <div class="rounded-md border">
      <Table>
        ...
        <TableBody>
          <TableEmpty v-if="loading" :colspan="4">A carregar IVA...</TableEmpty>
          ...
          <TableRow v-for="vat in vats" v-else :key="vat.id">
            ...
            <TableCell class="text-right">
              <div class="flex justify-end gap-2">
                <Button size="sm" variant="outline" :disabled="loading" @click="goToEdit(vat.id)">Editar</Button>
                <Button
                  size="sm"
                  variant="secondary"
                  :disabled="loading || togglingId === vat.id"
                  @click="onToggleStatus(vat)"
                >
                  {{ vat.is_active ? 'Inativar' : 'Ativar' }}
                </Button>
              </div>
            </TableCell>
          </TableRow>
